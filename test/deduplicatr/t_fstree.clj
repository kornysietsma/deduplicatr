...
(def throttle-forever (throttler/new-agent 100000000))
...
(with-redefs [df/file-summary stub-filesummaryfn]
  (fact "treeifying an empty directory produces mostly empty results"
    (treeify :group (fu/rel-path simple-fixture "parent" "child" "empty_grandchild") throttle-forever)
    => {:files []
        :dirs []
  ...
  (fact "treeifying a directory containg files produces a list of those files, and the directory summary"
    (treeify :group (fu/rel-path simple-fixture "ab") throttle-forever)
  ...
  (fact "treeifying a more complex directory structure produces a tree of summary information"
    (treeify :group (fu/rel-path simple-fixture "parent" "child") throttle-forever)
  ...
  (fact "the summary of a directory includes the accumulated summary from all descendant files"
    (:summary (treeify :group (fu/rel-path simple-fixture "parent") throttle-forever))
...
(fact "two directories with same contents have same hash and size"
   (let [tree1 (treeify :group (fu/rel-path simple-fixture "ab") throttle-forever)
         tree2 (treeify :group (fu/rel-path simple-fixture "parent" "child") throttle-forever)
...
(fact "two directories with same contents have same hash and size even if structure is different"
      (let [tree (treeify :group simple-fixture throttle-forever)
